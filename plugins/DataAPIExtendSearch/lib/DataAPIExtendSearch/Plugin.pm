...
sub data_api_content_data_extend {
    ...
    my $blog_id = $site->id;
    my $content_field_id = $content_type->id;
    ...
    my %terms = (blog_id => $blog_id, content_type_id => $content_field_id);
    my %args = (limit => $limit, offset => $offset);

    # Sort by a content field
    my $cf = MT->model('cf')->load( { blog_id => $blog_id, content_type_id => $content_field_id, name => $param_sort_field_name } );
    if ($cf) {
        my $data_type = MT->registry('content_field_types')->{ $cf->type }{data_type};
    ...
    return {
        ...
        paginate => $param_limit_per_page == 0 ? undef : {
